...
class TopoNode (object):
  ...
  def __init__ (self, portNum = 0, growPorts =  True):
    self.growPorts = growPorts
    self.entity = None
    self.ports = [None] * portNum
...
#    elif isinstance(cable, BidirectionalCable):
    ...
    if cable[0] is not None:
      ...
      #self.send(cs176.basics.DiscoverPackets(self.entity, True), localPort)
      
      # Get latency if c is BasicCable - Kaifei Chen(kaifei@berkeley.edu)
      l = c.latency if isinstance(c, BasicCable) else None  # latency
      self.send(cs176.basics.DiscoverPackets(self.entity, latency=l), localPort)

    if cable[1] is not None:
      c = fixCableEnd(cable[1], topoEntity, remotePort, self, localPort)
      topoEntity.ports[remotePort] = c

      #topoEntity.send(cs176.basics.DiscoverPackets(topoEntity.entity, True), remotePort)

      # Get latency if c is BasicCable - Kaifei Chen(kaifei@berkeley.edu)
      l = c.latency if isinstance(c, BasicCable) else None  # latency
      topoEntity.send(cs176.basics.DiscoverPackets(topoEntity.entity, latency=l), remotePort)
  ...
  def unlinkTo (self, topoEntity):
    ...
    def goDown (index):
      ...
      #topoEntity.entity.handle_rx(cs176.basics.DiscoverPackets(self.entity, False), otherPort)
      #self.entity.handle_rx(cs176.basics.DiscoverPackets(topoEntity.entity, False), index)
      
      # Assign infinity to latency - Kaifei Chen(kaifei@berkeley.edu)
      topoEntity.entity.handle_rx(cs176.basics.DiscoverPackets(self.entity, latency=float("inf")), otherPort)
      self.entity.handle_rx(cs176.basics.DiscoverPackets(topoEntity.entity, latency=float("inf")), index)
    ...
    for remote in ports:
      if remote >=0 and remote < len(self.ports):
        ...
        if remote is not None:
          p = copy.copy(packet)
          import basics
          if isinstance(packet, basics.UpdateRouting):
